...
const stripeRouter = createTRPCRouter({
  products: publicProcedure.query(async ({ ctx }) => {
    ...
    // sort by the smallest price
    const sortedProducts = products.sort((a, b) => {
      const aPrice = a.prices.sort((a, b) => a.unitAmount - b.unitAmount)[0];
      const bPrice = b.prices.sort((a, b) => a.unitAmount - b.unitAmount)[0];
  ...
  createCheckoutSession: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const quantity = ctx.team.userRoles.length || 1; // number of teammates
  ...
  syncAllProductsToDatabase: publicProcedure.mutation(async ({ ctx }) => {
  ...
  syncAllPricesToDatabase: publicProcedure.mutation(async ({ ctx }) => {
